...
/// <summary>
/// Covers moving what the company owes in end-of-service and unused leave into the ledger.
/// </summary>
/// <remarks>
...
public sealed class ProvisionPostingTests : IDisposable
{
        ...
        StringComparer.OrdinalIgnoreCase)
    {
        ...
        ["Hr.Posting.LeaveProvisionAccount"] = "2400",
    ...
    public async Task The_first_run_posts_the_whole_figure_as_the_movement()
    {
        ...
        var expected = await new EmployeeService(
                context, Catalog(), new StubNumbers(), setup, _clock, NullLogger<EmployeeService>.Instance)
            .EntitlementsAsync(Today);
        ...
        result.Value.EndOfServiceMovement.ShouldBe(expectedEndOfService);
        ...
        request.Lines.Count.ShouldBe(4);
        request.Lines.Sum(l => l.Amount).ShouldBe(0m, "a posting that does not balance is a defect");

        request.Lines.Single(l => l.AccountNo == "6110").Amount.ShouldBe(expectedEndOfService);
        request.Lines.Single(l => l.AccountNo == "2500").Amount.ShouldBe(-expectedEndOfService);
        request.Lines.Single(l => l.AccountNo == "6120").Amount.ShouldBe(expectedLeave);
        request.Lines.Single(l => l.AccountNo == "2400").Amount.ShouldBe(-expectedLeave);
    }
    ...
    public async Task Running_it_again_with_nothing_changed_posts_nothing()
    {
        ...
        result.Value.EndOfServiceMovement.ShouldBe(0m);
    ...
    public async Task A_later_run_posts_only_what_changed_since_the_last_one()
    {
        ...
        // A year passes: another year of service earns another year of end-of-service and leave,
        // on top of whatever the first run already carried.
        ...
        var later = await new ProvisionPostingService(
                second,
                new EmployeeService(
                    second, Catalog(), new StubNumbers(), setup, laterClock,
                    NullLogger<EmployeeService>.Instance),
                Catalog(),
                setup,
                events,
        ...
        later.Value.EndOfServiceTotal.ShouldBeGreaterThan(firstRun.Value.EndOfServiceTotal);
        later.Value.EndOfServiceMovement.ShouldBe(
            later.Value.EndOfServiceTotal - firstRun.Value.EndOfServiceTotal);

        var request = events.Published.ShouldHaveSingleItem();
        request.Lines.Single(l => l.AccountNo == "2500").Amount
            .ShouldBe(-later.Value.EndOfServiceMovement);
    }
    ...
    public async Task A_provision_with_no_account_set_up_is_refused_rather_than_posted_short()
    {
        ...
        {
            ["Hr.Posting.EndOfServiceAccount"] = null,
            ["Hr.Posting.EndOfServiceExpenseAccount"] = "6110",
            ["Hr.Posting.LeaveProvisionAccount"] = "2400",
            ["Hr.Posting.LeaveExpenseAccount"] = "6120",
        });
        ...
        refusal.Detail.ShouldNotBeNull().ShouldContain("Hr.Posting.EndOfServiceAccount");
