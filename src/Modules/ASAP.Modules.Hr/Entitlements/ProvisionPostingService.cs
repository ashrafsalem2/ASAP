...
/// <summary>What one provision run posted, and what the provision comes to now.</summary>
...
/// What the company owes in end-of-service, in total. Reported and not posted: the payroll run
/// charges that provision as it is earned. See <see cref="ProvisionPostingService"/>.
/// </param>
...
/// <summary>
/// Moves what the company owes in unused leave into the general ledger.
/// </summary>
/// <remarks>
/// <para>
/// Leave only, deliberately. The other entitlement -- end of service -- is charged by
/// <see cref="Payroll.PayrollService"/> as it is earned, split across the branches the month was
/// worked at, and posting it again here would book it twice. One writer per provision, the same
/// rule the rest of the system follows for control accounts. What this leaves undone is the
/// reconciliation: payroll's charge accumulates month by month and nothing yet compares the
/// running total against the entitlement formula, which is the follow-up recorded in the roadmap.
/// </para>
/// <para>
    ...
    ILogger<ProvisionPostingService> logger)
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var watermarks = await context.Set<EntitlementProvision>()
            .Where(p => p.Type == ProvisionType.Leave)
        ...
        if (lines.Count == 0)
        {
            found.Add(messages.Render(
                ...
                Args(("Amount", leaveTotal))));

            return Result<ProvisionPostingSummary>.Success(
                new ProvisionPostingSummary(day, leaveTotal, 0m, endOfServiceTotal, null),
                found);
        }
        ...
        logger.LogInformation(
            "Posted the leave provision as transaction {TransactionNo}: moved {LeaveMovement} "
            + "to {LeaveTotal}.",
            transactionNo,
            leaveMovement,
            leaveTotal);
        ...
        return Result<ProvisionPostingSummary>.Success(
            new ProvisionPostingSummary(
                day, leaveTotal, leaveMovement, endOfServiceTotal, transactionNo),
            found);
    }
